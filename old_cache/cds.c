...
void debug_print_cache(struct cache *c, char *cache_level)
{
    fprintf(debug_file, "%s %s cache: Total number of entries: %d\n", c->name, cache_level,  c->number_of_cache_entries);
    fprintf(debug_file, "%s %s cache: %s\n", c->name, cache_level,  print_sets_and_ways(c));
    fprintf(debug_file, "%s %s cache: Each cache line is %d bytes\n", c->name, cache_level,  c->cache_line_size);
    fprintf(debug_file, "%s %s cache: Cache is %s\n", c->name, cache_level,  c->write_back ? "write-back" : "write-thru");
    fprintf(debug_file, "%s %s cache: With a %s replacement policy\n", c->name, cache_level, CRP_name(c));
}


void debug_print_cds(CDS *cds)
{
    debug_print_cache(cds->c, "main");
    debug_print_cache(cds->v, "victim");
}
...
void Print_Cache_Statistics_for_one_cache(struct cache *c, char *cache_level)
{
    fprintf(stdout, "%s %s cache: %d entries of lines of %d bytes; %s, %s, %s\n",
            c->name, cache_level, c->number_of_cache_entries, c->cache_line_size,
            print_sets_and_ways(c),
            c->write_back ? "write-back" : "write-thru",
            CRP_name(c));
    
    fprintf(stdout, "%s %s cache: %d accesses, %d hits (%d%%), %d misses, %d miss reads, %d miss writes\n",
            c->name, cache_level, c->number_total_cache_access,
    ...
    if (c->write_back)
        fprintf(stdout, "%s %s cache: %d dirty cache lines remain\n", c->name, cache_level, number_dirty_lines(c, cache_level));
}
...
void Print_Cache_Statistics_for_one_cds(CDS *cds)
{
    ...
    Print_Cache_Statistics_for_one_cache(cds->c, "main");
    Print_Cache_Statistics_for_one_cache(cds->v, "victim");
...
void init_cache_for_trace(CDS *cds)
{
    ...
    cds->v->number_miss_reads = 0;
    cds->v->number_miss_writes = 0;
    cds->v->number_total_cache_access = 0;
    cds->v->number_cache_hits = 0;
    cds->v->number_cache_misses = 0;
}
