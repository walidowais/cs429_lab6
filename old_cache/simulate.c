...
int Search_Cache_For(struct cache *c, memory_address cache_address, char *cache_level)
{
    int cache_entry_index = Compute_Set_Index(c, cache_address);
    
    if (debug) fprintf(debug_file, "%s %s cache: search cache lines %d to %d for 0x%08X\n",
                       c->name, cache_level, cache_entry_index, 
...
/* search in the cache for the particular cache address we want */
int Search_Victim_For(struct cache *c, memory_address cache_address, char *cache_level)
{
    int cache_entry_index = Compute_Set_Index(c, cache_address);
...
int Find_Victim_by_Replacement_Policy(struct cache *c, memory_address cache_address, char *cache_level)
{
    int i;
    int victim;
    ...
    if (debug) fprintf(debug_file, "%s %s cache: look for victim in %d lines starting at %d\n", c->name, cache_level,  set_size, first_index);
            ...
            if (!(c->c_line[first_index+i].valid))
                {
                    ...
                    if (debug) fprintf(debug_file, "%s %s cache: found empty cache entry at %d\n", c->name, cache_level,  victim);
                    return(victim);
                }
        }
    ...
    switch (c->replacement_policy)
        ...
        case CRP_LFU:  /* replacement data is the number of uses, so
                      choose the smallest */
            {
                ...
                if (debug) fprintf(debug_file, "%s %s cache: replacement data: [%d, 0x%08X]: %d", c->name, cache_level, victim, c->c_line[victim].tag, min);
    ...
    if (debug) fprintf(debug_file, "%s %s cache: found victim in entry %d\n", c->name, cache_level,  victim);
    return(victim);
}


void evict_dirty_line_from_cache(struct cache *c, cache_line *victim_line, char *cache_level)
{
    if (debug) fprintf(debug_file, "%s %s cache: Write dirty victim 0x%08X\n", 
                       c->name, cache_level,  victim_line->tag);
...
Boolean evict_from_cache(CDS *cds, cache_line *victim_line, memory_address cache_address)
{
    ...
    int victim_entry_index;
    //832
    /* if victim is dirty, note that this dirty line is being evicted */
    if (victim_line->dirty)
        {
            evict_dirty_line_from_cache(cds->c, victim_line, "main");
        }

    //832
    victim_entry_index = Find_Victim_by_Replacement_Policy(cds->v, cache_address, "victim");
    cache_entry = &(cds->v->c_line[victim_entry_index]);
    cds->v->c_line[victim_entry_index] = *victim_line;
...
void Simulate_Reference_to_Cache_Line(CDS *cds, memory_reference *reference)
{
    ...
    memory_address victim_address = Base_Cache_Address(cds->v, reference->address);

    int cache_entry_index = Search_Cache_For(cds->c, cache_address, "main");
    ...
    else if(vsed && ((victim_entry_index = Search_Victim_For(cds->v, cache_address, "victim")) >= 0))
        {
            fprintf(stderr, "%x\n", cache_address);   
            cds->vic_hits++;
            // cds->c->number_cache_misses++;
            // fprintf(stderr, "after--%d--\n", cds->vic_hits);
            Found = TRUE;

            cache_entry_index = Find_Victim_by_Replacement_Policy(cds->c, cache_address, "main");
            // victim_entry_index = Find_Victim_by_Replacement_Policy(cds->v, victim_address, "victim");
            cache_entry = &(cds->c->c_line[cache_entry_index]);
            
            temp = &(cds->v->c_line[victim_entry_index]);
            cds->v->c_line[victim_entry_index] = cds->c->c_line[cache_entry_index];
            cds->c->c_line[cache_entry_index] = *temp;

            Set_Replacement_Policy_Data(cds->number_of_memory_reference, cds->v, &(cds->v->c_line[victim_entry_index]));
...
int number_dirty_lines(struct cache *c, char *cache_level)
{
    int n = 0;
    int i;
    for (i = 0; i < c->number_of_cache_entries; i++)
        {
            if (c->c_line[i].dirty)
                {
                    n += 1;
                    if (debug) fprintf(debug_file, "%s %s cache: Cache Line 0x%08X is dirty\n", c->name, cache_level, c->c_line[i].tag);
                }
        }
    return(n);
}
