...
void Print_Cache_Statistics_for_one_victim(struct cache *c)
{
    fprintf(stdout, "%s victim cache: %d entries of lines of %d bytes; %s, %s, %s\n",
            c->name, c->number_of_cache_entries, c->cache_line_size,
            print_sets_and_ways(c),
            c->write_back ? "write-back" : "write-thru",
            CRP_name(c));
    
    fprintf(stdout, "%s victim cache: %d accesses, %d hits (%d%%), %d misses, %d miss reads, %d miss writes\n",
    ...
    if (c->write_back)
        fprintf(stdout, "%s victim cache: %d dirty cache lines remain\n", c->name, number_dirty_lines(c));
}

void Print_Cache_Statistics_for_one_with_victim(struct cache *c)
{
    fprintf(stdout, "%s main cache: %d entries of lines of %d bytes; %s, %s, %s\n",
            c->name, c->number_of_cache_entries, c->cache_line_size,
            print_sets_and_ways(c),
            c->write_back ? "write-back" : "write-thru",
            CRP_name(c));
    
    fprintf(stdout, "%s main cache: %d accesses, %d hits (%d%%), %d misses, %d miss reads, %d miss writes\n",
            c->name, c->number_total_cache_access,
            c->number_cache_hits, percent(c->number_cache_hits, c->number_total_cache_access),
            c->number_cache_misses, c->number_miss_reads, c->number_miss_writes);
    
    if (c->write_back)
        fprintf(stdout, "%s main cache: %d dirty cache lines remain\n", c->name, number_dirty_lines(c));
}
