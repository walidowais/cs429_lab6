...
void define_key_value_pair(CDS *cds, Token *key, Token *value)
{
    ...
    /* look for victim size */
    if (strcasestr(key->string, "victim") != NULL)
        {
            int n = atoi(value->string);
            cds->c->num_vic_lines = n;
            return;
        }
...
CDS *Read_CDS_file_entry(FILE *CDS_file)
{
    ...
    cds->v->cache_line_size = cds->c->cache_line_size;
    cds->v->number_of_cache_entries = cds->c->num_vic_lines;
    cds->v->number_of_ways = cds->c->num_vic_lines;
    cds->v->write_back = cds->c->write_back;
    cds->v->replacement_policy = CRP_FIFO;
    cds->v->LFU_Decay_Interval = 200000;
    cds->v->c_line = NULL;
    cds->v->num_vic_lines = 0;
    cds->v->is_vic = TRUE;
