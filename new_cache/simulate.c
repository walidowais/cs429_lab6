...
int Search_Cache_For(struct cache *c, memory_address cache_address)
{
    ...
    if(c->is_vic){
        ...
        cache_entry_index = Compute_Set_Index(c, cache_address);
    }
...
int Find_Victim_by_Replacement_Policy(struct cache *c, memory_address cache_address)
{
    ...
    if(c->is_vic){
        ...
        first_index = Compute_Set_Index(c, cache_address);
    }
    ...
    else
        {
            ...
            if (cache_entry->valid){
                ...
                if(cds->c->num_vic_lines > 0){
                    ...
                    int v_cache_entry_index = Search_Cache_For(cds->v, cache_address);

                    if(v_cache_entry_index >= 0){
                        Found = TRUE;
                        swap_cache_lines(cache_entry, &(cds->v->c_line[v_cache_entry_index]));
                        Update_Replacement_Policy_Data(cds->number_of_memory_reference, cds->c, &(cds->v->c_line[v_cache_entry_index]));
                    }
                    else{
                        cds->v->number_miss_reads += 1;

                        v_cache_entry_index = Find_Victim_by_Replacement_Policy(cds->v, cache_address);

                        if(cds->v->c_line[v_cache_entry_index].dirty){
                            cds->v->number_miss_writes += 1;
                        }

                        cds->v->c_line[v_cache_entry_index] = *cache_entry;
                        Update_Replacement_Policy_Data(cds->number_of_memory_reference, cds->c, &(cds->v->c_line[v_cache_entry_index]));
